...
//Sensor at x=20, y=1: closest beacon is at x=15, y=3
  const coordinates = data.trim().split('\n').map(item => {
    ...
    xMin = Math.min(xMin, coords.sensor.x, coords.beacon.x);
    xMax = Math.max(xMax, coords.sensor.x, coords.beacon.x);
    yMin = Math.min(yMin, coords.sensor.y, coords.beacon.y);
    yMax = Math.max(yMax, coords.sensor.y, coords.beacon.y);
  ...
  console.log({xRange: xMax - xMin, yRange: yMax-yMin, xMin, xMax, columns})
